...
init(_) ->
    ...
    ?DEBUG("rds_la_protocol initialized, pid ~p~n", [self()]),
    {ok, #la_pstate{}}.

terminate(_Reason, _ModState) ->
    ?DEBUG("rds_la_protocol terminate, pid ~p~n", [self()]),
    ok.
...
handle_nowait(ModState = #la_pstate{tmpq = TmpQ, last_state = LastState}) ->
    case queue:out(TmpQ) of
        ...
        {{value, {From, Term}}, NTmpQ} ->
            ...
            %?PERF_QPS_SCOPE(handle_fsm, start, "rds_la_protocol handle_fsm"),
            Res = handle_fsm(LastState, From, Term, ModState#la_pstate{tmpq = NTmpQ}),
            %?PERF_QPS_SCOPE(handle_fsm, stop, "rds_la_protocol handle_fsm"),
            Res
    end.
...
handle_local_info(LocalInfo, ModState = #la_pstate{tmpq = TmpQ}) ->
    ...
    %?PERF_QPS_SCOPE(handle_local_info, start, "rds_la_protocol handle_local_info"),
    Res = handle_nowait(ModState#la_pstate{tmpq = QIn}),
    %?PERF_QPS_SCOPE(handle_local_info, stop, "rds_la_protocol handle_local_info"),
    Res.
...
do_serialize(Term) ->
    %?PERF_QPS_SCOPE(do_serialize, start, "rds_la_protocol do_serialize"),
    Binary = term_to_binary(Term),
    %?PERF_QPS_SCOPE(do_serialize, stop, "rds_la_protocol do_serialize"),
    Binary.

do_deserialize(Binary) ->
    %?PERF_QPS_SCOPE(do_deserialize, start, "rds_la_protocol do_deserialize"),
    Res = try binary_to_term(Binary) of
        Term -> Term
    catch
        _:_ -> {error, illedge_binary}
    end,
    %?PERF_QPS_SCOPE(do_deserialize, stop, "rds_la_protocol do_deserialize"),
    Res.
...
handle_command({append, Key, Value}, ModState = #la_pstate{storage = Storage}) ->
    ...
    %?PERF_QPS_SCOPE(dict_store, start, "rds_la_protocol dict_store"),
    %Stored = Storage,
    %?PERF_QPS_SCOPE(dict_store, stop, "rds_la_protocol dict_store"),
...
handle_command({lookup, Key}, ModState = #la_pstate{storage = Storage}) ->
    %?PERF_QPS_SCOPE(dict_find, start, "rds_la_protocol dict_find"),
    Res = dict:find(Key, Storage),
    %?PERF_QPS_SCOPE(dict_find, stop, "rds_la_protocol dict_find"),
